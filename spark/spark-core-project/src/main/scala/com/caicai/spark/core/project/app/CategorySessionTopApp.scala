package com.caicai.spark.core.project.app

import com.caicai.spark.core.project.bean.{CategoryCountInfo, SessionInfo, UserVisitAction}
...
object CategorySessionTopApp {
    ...
    def statCategorySessionTop10_2(sc: SparkContext, categoryTop10: Array[CategoryCountInfo], userVisitActionRDD: RDD[UserVisitAction]) = {
        ...
        val temp = cids.map(f = cid => {
            ...
            val cidUserVisitActionRDD: RDD[UserVisitAction] = {
    ...
    */
    ...
    def statCategorySessionTop10_4(sc: SparkContext, categoryTop10: Array[CategoryCountInfo], userVisitAction: RDD[UserVisitAction]): Unit = {
        val cids = categoryTop10.map(_.categoryId.toLong)
        val filteredUserVisitActionRDD = userVisitAction.filter(action => cids.contains(action.click_category_id))
        val cidSidAndOne = filteredUserVisitActionRDD.map(action => ((action.click_category_id, action.session_id), 1))
        val cidSidAndCount = cidSidAndOne.reduceByKey(new MyHashPartitioner(cids), _ + _)
    ...
    class MyHashPartitioner(cids: Array[Long]) extends Partitioner{
        ...
        override def getPartition(key: Any): Int = {
            key match{
                ...
                case (cid:Long,_) =>
                    val s  = cidIndexMap(cid)
                    s
                case _=> throw  new UnsupportedOperationException("错误的示范")
            }
        }
