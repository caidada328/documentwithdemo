package com.caicai.gmall.common

import java.util

import io.searchbox.client.config.HttpClientConfig
import io.searchbox.client.{JestClient, JestClientFactory}
import io.searchbox.core.{Bulk, Get, Index}
   ...
   //new Get
    ...
    case data =>
      ...
      bulk.addAction(action)
        ...
        client.shutdownClient() // extract rdd to the driver-end
    }

    def insertBulk2 (index:String,sources:Iterator[Any]) ={
...
object ESUtil1{
 ...
 def getClient = factory.getObject
    ...
    def insertBulk(index:String,sources:util.Iterator[Any]) = {
        val client = factory.getObject
        val bulk = new Bulk.Builder()
          .defaultIndex(index)
          .defaultType("_doc")
